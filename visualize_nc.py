...
host_image = False  # Set to true if you want to see the website on a locally hosted website
directory = "data"
output_dir = "images"
...
def convert_nc_to_images(directory, output_dir):
    """Converts all .nc files in the directory to images and saves them in the output directory."""
...
def create_image_server(image_files, output_dir):
    """Sets up a Flask web server to display images generated from .nc files."""
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    @app.route('/')
    def index():
        abi_images = [os.path.basename(img)
                      for img in image_files if "ABI" in img]
        glm_images = [os.path.basename(img)
                      for img in image_files if "GLM" in img]
...
if __name__ == '__main__':
    image_files = convert_nc_to_images(directory, output_dir)
    # The create_image_server function is ready to be called when needed
    if host_image:
      create_image_server(image_files, output_dir)
